...
for (var i = 0; i < response.length; i++) {
	alert(response[i].produkt.length);		
...
'<thead><tr><th id="a'+i+'" class="one"></th></tr><tr><th class="two">Klient</th><th class="two">Kraj</th><th class="two">Zespół Produkcyjny</th><th class="two">Data zakończenia</th>'+
'</tr></thead><tbody id="supply_order'+i+'"></tbody>');


$('#a'+i).append(response[i].zs);

$('#supply_order'+i).append("<tr><td>" + response[i].zs + "</td><td data-name='employee_name' class='employee_name' data-type='select' id='status'>" 
+ response[i].produkt['numer'] + "</td><td data-name='employee_name' class='employee_name' data-type='select' id='status'>" 
+ response[i].zs + "</td><td data-name='employee_salary' class='employee_salary' data-type='text'>" 
+ response[i].zs 
+ "</td></tr></table>");
		...
		$.ajax({
		  ...
		  success: function(response2)  
		  {
			...
			for (var j = 0; j < response2.length; j++) {
				...
				//	alert(cnt);
//	$('#supply_device'+j).append('<tr><td>'+response2[row].productid+'</td><td>'+response2[row].serialNumber+'</td></tr>');
...
$('#supply_deviceDetails'+cnt).append('<tr><td>'+response2[j].productid+'</td></tr>');*/
...
getDevice(response[i].orderId,i);
